...
class ImageController extends Controller
{
    ...
    public function __construct(Request $request)
    {
		...
		$this->diskRaw = Storage::disk('sources');
    }
    ...
    public function upload(Request $request)
    {
        ...
        if ($request->hasFile($fileBlob)) {
			...
			$sourceDir = storage_path('app'.DIRECTORY_SEPARATOR.'sources'.DIRECTORY_SEPARATOR);
			
            $uniqFileName = uniqid() . '.' . $fileExtension;
            $targetFile = $targetDir . '/' . $uniqFileName;              // The target file path
			$targetFilePath = storage_path('app'.DIRECTORY_SEPARATOR.'public'.DIRECTORY_SEPARATOR.$targetDir . '/' . $uniqFileName);
            //$sourceFile = $sourceDir . 'sources/' . $uniqFileName;              // The target file path
            $sourceFile = 'sources/' . $uniqFileName;              // The target file path
            ...
            if ($totalChunks == 1 || ($totalChunks > 1 && $allChunksUploaded)) {
                //if ($this->disk->put($sourceFile, file_get_contents($file))) {
                    // Delete the temp directory if exist
					
					$imgFile = Image::make(file_get_contents($file));
					$imgFile->resize(1000, 1000, function ($constraint) {
						$constraint->aspectRatio();
					})->save($targetFilePath);
					
					$this->diskRaw->put($uniqFileName, file_get_contents($file));
     ...
     * @return json
     */
    public function delete(Request $request, ImageModel $image)
    {
        $image->delete();
    ...
    {
        $order = $request->all();
        $images = ImageModel::find(array_keys($order));
